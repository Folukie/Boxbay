            ...
            <div className={"flex justify-between text-black gap-10 sm:grid sm:grid-cols-1 sm:gap-y-20"}>
                {service.map((service_data) => (
                    <div key={service_data.id}
                        ...
                        <div
                            className={"absolute -top-7 left-36 bg-white py-4 px-6 shadow-sm rounded-tl-2xl rounded-br-2xl sm:left-32"}>
                            <Image src={service_data.icon} width={30} height={30} objectFit={"contain"}/>
                        </div>
                        <h2 className={"text-med font-bold pt-10 sm:text-mobilebase"}>{service_data.title}</h2>
